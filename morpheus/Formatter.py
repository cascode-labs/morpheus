...
class moprheus_Formatter_OBJECTS:
    def format(self, format_object, /, *args, **kwargs):
        return self.vformat(format_object, args, kwargs,returnlist=True)

    def vformat(self, format_object, args, kwargs,returnlist = False):
        used_args = set()
        result, _ = self._vformat(format_object, args, kwargs, used_args, 2,returnlist=returnlist)
        self.check_unused_args(used_args, args, kwargs)
        return result

    def _vformat(self, format_object, args, kwargs, used_args, recursion_depth,
                 auto_arg_index=0,returnlist=False):
        if recursion_depth < 0:
            raise ValueError('Max string recursion exceeded')
        result = []
        ...
        for key,value in obj.__dict__.items():
            [objects, total_fields] = self.createObjects(total_fields)
